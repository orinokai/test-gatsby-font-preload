...
async function main() {
  ...
  const sections = [dim(` crawling routes`), `:bar`, bold(blue(`:percent`)), dim(`eta :etas`), ``];
  ...
  page.on(`requestfinished`, req => {
    ...
    if (req.method() === `GET` && req.resourceType() === `font`) {
      ...
      const fontUrl = isSelfHosted ? req.url().slice(devServer.length) : req.url();
  ...
  for (const route of routes) {
    ...
    await page.goto(endpoint(route), {
  ...
  await browser.close();
  logger.resetAdapter();
  cache.hash = (0, _gatsbyCoreUtils.createContentDigest)(routes);
